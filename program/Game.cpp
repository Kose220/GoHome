...
void Game::Update()
{
	...
	if (game) {
		...
		case STATUS_UFO_OUT:
			...
			if (is_ufo_anim)
			{
				status_ = STATUS_IDLE;
				next_anim = model_anim_idle;
				//	1度だけ使用されるように使われたらboolをfalseに戻す
				is_ufo_anim = false;
			}
			break;
		}
	...
	//						player.move_x += player.move_y;
			...
			River_SE_Volume = River_distance;
		...
		//	プレイヤーの座標についたらトラッキングを解除し次の処理へ向かう
		if (ufo.pos_x == player.pos_x)
		{
			ufo.Tracking = false;
		}
		...
		if (ufo.ufo_die) {
			...
			//	プレイヤーがUFOステージにつき次第UFOのライトが展開される
...
//	DrawSphere3D({ 1.0f, 1.0f,player.pos_z + 10.0f },5.0f,50.0f,RED,RED,true);
	...
	//  wallの描画
		...
		ui.Draw();
		if (ufo.ufo_die) {
			DrawFormatStringF(WINDOW_W / 2 - 350, WINDOW_H - 135, GetColor(255, 255, 255), "UFOに捕まった！Iキーを連打して脱出！\n              10/%d", ufo_rampage);
		}
		//if(ufo.ufo_standhing) {
		//    //  仮でUFOの座標を表示
//   DrawFormatString(WINDOW_W / 2, WINDOW_H / 2, GetColor(255, 255, 255), "UFOに捕まった！Iキーを連打して脱出！\n              %f", ufo.pos_x);
		//}
		////  仮でプレイヤーの座標を表示
